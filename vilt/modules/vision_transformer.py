...
import vilt.modules.moil as moil
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, val_mode=None, I=None):
        ...
        def reshape(_q, _k, _v):
            q = _q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            k = _k(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            v = _v(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            return q, k, v
        
        def reparameterize(m_val, m_init, m_lora):
